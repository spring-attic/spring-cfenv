...
public abstract class AbstractJdbcUrlCreator implements JdbcUrlCreator {
	...
	protected UriInfo getUriInfo(CfCredentials cfCredentials, String uriScheme) {
		String uri = cfCredentials.getUri(uriScheme);
		UriInfo uriInfo;
		if (uri == null) {
			String hostname = cfCredentials.getHostname();
			String port = cfCredentials.getPort();
			String username = cfCredentials.getUsername();
			String password = cfCredentials.getPassword();
			String databaseName = cfCredentials.getName();
